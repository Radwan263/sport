...
import { ShoppingBag, Loader2, Search, Filter, Plus, Heart } from "lucide-react"; // ✅ ضفنا Heart
import { Input } from "@/components/ui/input";
import { useLocation } from "wouter";
import { useCart } from "@/contexts/CartContext";
import { useWishlist } from "@/contexts/WishlistContext"; // ✅ استدعاء هوك المفضلة

export default function Shop() {
  const [, navigate] = useLocation();
  const { addItem, totalItems } = useCart();
  const { toggleWishlist, isInWishlist } = useWishlist(); // ✅ دوال المفضلة
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredProducts.map((product) => (
              <div 
                ...
                className="bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all cursor-pointer border border-slate-100 group relative"
              >
                {/* ✅ زرار القلب هنا */}
                <button
                  onClick={(e) => {
                    e.stopPropagation(); // عشان ميفتحش صفحة المنتج لما ندوس قلب
          ...
          <div className="text-center py-20 bg-white rounded-3xl border border-dashed mx-4">
            ...
            <h3 className="text-xl font-bold text-slate-400">لا توجد منتجات</h3>
            <p className="text-slate-500 text-sm mt-2">جرب البحث بكلمة أخرى</p>
          </div>
        )}
      </div>
    </div>
